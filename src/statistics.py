...
class StatisticsTracker:
    ...
    def reset(self):
        ...
        self.team_stats = defaultdict(lambda: {
            ...
            'total_deaths': 0,
            'agents_alive': 0,
            'total_shots_fired': 0,  # Total bullets fired by team
            'final_dps': None  # DPS frozen when game ends
        })
    ...
    def on_agent_spawn(self, agent):
        ...
        self.agent_stats[agent_id]['team_id'] = agent.team_id  # Stocăm team_id
    ...
    def on_shot_fired(self, agent):
        ...
        self.team_stats[agent.team_id]['total_shots_fired'] += 1
    ...
    def get_team_avg_time_alive(self, team_id):
        """Returnează timpul mediu de viață pentru agenții unei echipe"""
        team_times = [stats['time_alive'] for stats in self.agent_stats.values()
                     if stats['team_id'] == team_id and stats['time_alive'] > 0]
        if not team_times:
            return 0.0
        return sum(team_times) / len(team_times)
   
    def get_team_avg_distance(self, team_id):
        """Returnează distanța medie parcursă de agenții unei echipe"""
        team_distances = [stats['distance_traveled'] for stats in self.agent_stats.values()
                         if stats['team_id'] == team_id]
        if not team_distances:
            return 0.0
        return sum(team_distances) / len(team_distances)
   
    def get_team_dps(self, team_id):
        """Returnează DPS pentru o echipă (damage total / timp total)"""
        # Returnează valoarea înghețată dacă există (după sfârșitul jocului)
        if self.team_stats[team_id]['final_dps'] is not None:
            return self.team_stats[team_id]['final_dps']
       
        # Altfel calculează în timp real
        elapsed = (pygame.time.get_ticks() - self.start_time) / 1000.0
        if elapsed == 0:
            return 0.0
        return self.team_stats[team_id]['total_damage_dealt'] / elapsed
   
    def freeze_dps(self):
        """Îngheață valorile DPS când jocul se termină"""
        elapsed = (pygame.time.get_ticks() - self.start_time) / 1000.0
        for team_id in [0, 1]:
            if elapsed > 0:
                self.team_stats[team_id]['final_dps'] = self.team_stats[team_id]['total_damage_dealt'] / elapsed
            else:
                self.team_stats[team_id]['final_dps'] = 0.0
   
    def get_team_total_shots(self, team_id):
        """Returnează totalul de gloanțe trase de echipă"""
        return self.team_stats[team_id]['total_shots_fired']
